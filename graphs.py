...
def graph2():
    ...
    cursor = db.cursor()
    cursor.execute("""SELECT SCORE,DEATH FROM IISC WHERE DEATH IS NULL""")
...
def graph3():
    db = mdb.connect("localhost","root","wrongpassword","WORDS" )
    cursor = db.cursor()
    cursor.execute("""SELECT SCORE,DEATH FROM IISC WHERE DEATH IS NOT NULL""")
    results = cursor.fetchall()
    score = []
    scorerange = []
    startscore = 0
...
def graph4():
    db = mdb.connect("localhost","root","wrongpassword","WORDS" )
    cursor = db.cursor()
    cursor.execute("SELECT BIRTH_UN,SUM(SCORE) as TOTAL from IISC group by BIRTH_UN ORDER BY TOTAL DESC")
    results = cursor.fetchall()
...
def graph5():    
    db = mdb.connect("localhost","root","wrongpassword","WORDS" )
    cursor = db.cursor()
    cursor.execute("SELECT BIRTH,DEATH FROM IISC")
    results = cursor.fetchall()
...
def graph6():
    db = mdb.connect("localhost","root","wrongpassword","WORDS" )
    cursor = db.cursor()
    cursor.execute("select DISTINCT BIRTH_UN,BIRTH FROM IISC")
    results = cursor.fetchall()
    uniquename = []
    uniquedate = []
    convertdate = []
    ...
    '''for i in range(len(uniquename)):
        print uniquename[i],uniquedate[i]
    '''
...
def graph7():
    db = mdb.connect("localhost","root","wrongpassword","WORDS" )
    cursor = db.cursor()
    cursor.execute("SELECT BIRTH_UN, SUM(score) as TOTAL from IISC group by BIRTH_UN ORDER BY TOTAL DESC LIMIT 5")
    results = cursor.fetchall()
    usernames = []
    for i in results:
    ...
    for nms in usernames:
        ...
        deathdate = []
        clubbed = []
        cursor.execute("""SELECT DEATH,SCORE from IISC where BIRTH_UN=%s""",(nms))
        results = cursor.fetchall()
