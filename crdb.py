...
def Datastore(search):
    ...
    cursor = db.cursor()
    cursor.execute("DROP TABLE IF EXISTS IISC")
    cursor.execute("CREATE TABLE IISC(WORD VARCHAR(200) NOT NULL, BIRTH_UN VARCHAR(60), DEATH_UN VARCHAR(60), BIRTH VARCHAR(60), DEATH VARCHAR(60), SCORE REAL(8,2))")
    ...
    for lines in text:
        ...
        if(l1[0]=="Added"):
            try:
                cursor.execute("""INSERT INTO IISC(WORD, BIRTH_UN, BIRTH) VALUES(%s, %s, %s)""",(l1[1], l1[2], l1[3]))
            except Exception, e:
                print e , l1[1]
        elif(l1[0]=="Deleted"):
            cursor.execute("""SELECT * FROM IISC WHERE WORD=%s""",(l1[1]))
            ...
            if results:
                ...
                if len(l1)>3:
                    cursor.execute("""UPDATE IISC SET DEATH_UN = %s WHERE WORD = %s AND BIRTH = %s""",(l1[2], word , last_birth))
                    cursor.execute("""UPDATE IISC SET DEATH = %s WHERE WORD = %s AND BIRTH = %s""",(l1[3], word , last_birth))
...
def Score():
    db = mdb.connect("localhost","root","wrongpassword","WORDS" )
    cursor = db.cursor()
    cursor.execute('SELECT * FROM IISC')
    results = cursor.fetchall()
    for row in results:
        #print row[3], row[4]
        ...
        if row[4] != None:
            ...
            cursor.execute("""UPDATE IISC SET SCORE = %s WHERE DEATH = %s AND BIRTH = %s AND WORD = %s""",(score, row[4], row[3], row[0]))
            
        else:
            death = datetime.datetime.now()
            score = (death - birth).days
            cursor.execute("""UPDATE IISC SET SCORE = %s WHERE BIRTH = %s AND WORD = %s""",(score,row[3],row[0]))
...
def Extractscore():
    db = mdb.connect("localhost","root","wrongpassword","WORDS" )
    cursor = db.cursor()
    cursor.execute("""SELECT * FROM IISC WHERE DEATH IS NULL""")
    results = cursor.fetchall()
    alive_score = []
    alive_words = []
    dead_score = []
    ...
    cursor.execute("""SELECT * FROM IISC WHERE DEATH IS NOT NULL""")
    results = cursor.fetchall()
    if results:
        for row in results:
...
def Userscore():
    ...
    cursor.execute("""SELECT * FROM IISC WHERE BIRTH_UN = %s""",(user_name))
    results = cursor.fetchall()
    u_score = 0
    if results:
        for row in results:
